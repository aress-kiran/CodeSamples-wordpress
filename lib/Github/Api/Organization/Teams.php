...
{
    public function all($organization, $page = 1, $per_page = 100 )
    {
        return $this->get('orgs/'.rawurlencode($organization).'/teams', array(
						'page' => $page,
						'per_page' => $per_page
        ));
    }
    ...
    public function removeRepository($team, $username, $repository)
    {
		...
		public function getTeamMembership( $team, $username ) {
			return $this->put('teams/'.rawurlencode($team).'/memberships/'.rawurlencode($username));
		}
		
		public function addTeamMembership( $team, $username ) {
			return $this->put('teams/'.rawurlencode($team).'/memberships/'.rawurlencode($username));
		}
		
		public function removeTeamMembership( $team, $username ) {
